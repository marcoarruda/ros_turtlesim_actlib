...
class AveragingAction {
  ...
  public:
    ...
    AveragingAction(std::string name) : as_(nh_, name, false), action_name_(name) {
      ...
      as_.registerPreemptCallback(boost::bind(&AveragingAction::preempCB, this));
  ...
  protected:
    ...
    ros::NodeHandle nh_;
    actionlib::SimpleActionServer<actlib::AveragingAction> as_;
    std::string action_name_;
    int data_count_, goal_;
    float sum_, sum_sq_;
    actlib::AveragingFeedback feedback_;
    actlib::AveragingResult result_;
    ros::Subscriber sub_;
